...
//部分文件不添加md5时间戳(validator-bug)：dev命令需要注释，否则不生成local文件夹
fis.config.set('roadmap.path', [
    {
        reg: /.*\/validator\/local\/(.*\.(?:js|css))/,
        useHash: false,
        release : '../__dist/appmanager-pc$&'
    },{
        reg: /.*\/validator\/(.*\.(?:css))/,
        useHash: false,
        release : '../__dist/appmanager-pc$&'
...
var project = '/appmanager-pc'
fis.config.merge({
    statics : project + '/static',
    roadmap : {
        // domain : '//stgs1.yyfq.com'
        domain : '//s1.yyfq.com',
        // domain:'//172.28.3.53'
    }
});
